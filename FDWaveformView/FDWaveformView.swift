...
open class FDWaveformView: UIView {
    ...
    @available(*, deprecated, message: "Use `highlightedSamples` to set range")
    @objc open dynamic var progressSamples: Int {
        get {
...
extension FDWaveformView: UIGestureRecognizerDelegate {
    ...
    @objc func handlePanGesture(_ recognizer: UIPanGestureRecognizer) {
        ...
        if doesAllowScroll {
            ...
            let xLocation = min(max(recognizer.location(in: self).x, 0), frame.width)
            highlightedSamples = 0 ..< Int((CGFloat(zoomSamples.startIndex) + rangeSamples * xLocation / bounds.width))
